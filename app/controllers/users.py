...
from users.serializers import UserSerializer, GroupSerializer, UserRestrictSerializer, AllPermissionSerializer, \
    GroupPermissionSerializer, PermissionSerializer, UserCountriesSerializer, CountriesSerializer
import logging
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializeObj.is_valid():
            ...
            user_obj.save()
            #logger.info('User Data: ' + str(serializeObj.data))
        ...
        if serializeObj.is_valid():
            ...
            if user_countries is not None and len(user_countries) > 0:
                ...
                for item in user_countries:
                    ...
                    logger.info('Fetching user Country ID: ' + str(item.get('id')) + ' UserID: ' + str(pk))
                    try:
                        userCountries = UserCountries.objects.get(country_name_id=item.get('id'), user_name_id=pk)
...
class CountriesViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        candidateObj = Countries.objects.all()
