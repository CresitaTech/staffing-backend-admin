...
from jobdescriptions.models import jobModel, jobSubmissionModel, jobAssingmentModel
from jobdescriptions.serializers import JobDescriptionSerializer,UserDropDownListSerializer, JobAssingmentSerializer, JobSubmissionSerializer , JobDescriptionWriteSerializer ,JobAssingmentWriteSerializer ,JobSubmissionWriteSerializer
...
from jobdescriptions.utils import render_to_pdf,sendAssingmentMail
    ...
    def create(self, request):
        ...
        if serializeObj.is_valid():
            ...
            if serializeObj.data['secondary_recruiter_email'] != '' and serializeObj.data['secondary_recruiter_email'] != None:
                ...
                setattr(mail_primary, 'cc_email',[serializeObj.data['assignee_email'].strip() , BDM[2].strip()])
                ...
                setattr(mail_secondary, 'cc_email', [serializeObj.data['assignee_email'].strip() , BDM[2].strip()])
...
class GetUserList(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        "auth_group as ag , users_user_groups as g WHERE g.user_id = u.id AND g.group_id = ag.id AND (ag.name = 'RECRUITER' OR ag.name = 'RECRUITER MANAGER')")
