...
from reports.serializers import RecruiterPerformanceSummarySerializer, WeekendEmailReportSerializer, \
    ...
    BdmJobsAssignmentReportSerializer, JobSummaryCSVSerializer
...
def send_recruiter_email(country='India', isProd=False):
    ...
    if queryset is not None:
        ...
        userqueryset = User.objects.raw("SELECT id, email, country from users_user where role = '3' AND is_active = '1' AND country IN %s",
                                        [user_countries])
        ...
        bdmqueryset = User.objects.raw(
            "SELECT id, email, country from users_user where role = '9' AND is_active = '1' AND country = %s", [country])
        ...
        email = EmailMessage(subject="Daily submissions for {0}".format(start_date), body=message,
                             from_email='osms.opallios@gmail.com', to=recruiter_emails)  # recruiter_emails
...
def send_bdm_email(country='India', isProd=False):
    ...
    if queryset is not None:
        ...
        userqueryset = User.objects.raw(
            "SELECT id, email, country from users_user where role = '9' AND is_active = '1' AND country IN %s", [user_countries])
        user_serializer = UserSerializer(userqueryset, many=True)
        bdm_emails = []
        jobs_posted = []
        if isProd:
        ...
        email = EmailMessage(subject="Daily Jobs for {0}".format(start_date), body=message,
                             from_email='osms.opallios@gmail.com', to=bdm_emails)  # bdm_emails
        email.content_subtype = 'html'
        if isProd:
            email.cc = ['mathurp@opallios.com', 'girish@opallios.com', 'paradkaro@opallios.com',
                        'minglaniy@opallios.com',
...
def send_recruiter_summary_report(country='India', isProd=False):
    today_date = utils.get_current_datetime()
    ...
    if queryset is not None:
        ...
        message = render_to_string('recruiter_summary_report.html', {'data': finalOutput, 'no_submission': submission_output})
        email = EmailMessage(subject="Summary of Candidates Added By Recruiter: {0} to {1}".format(str(start_date).split(' ')[0],
...
def send_weekly_recruiter_submission(request, country='India', isProd=False):
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    start_date, end_date = utils.get_weekly_report_start_and_end_datetime(country)
    ...
    queryset = Candidates.objects.raw(
        ...
        "recruiter_name , u2.country as location , j.job_title, cjs.submission_date, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate, ca.created_at  FROM "
    ...
    if queryset is not None:
        ...
        message = render_to_string('recruiter_weekly_submission_report.html', {'data': finalOutput, 'bdm_emails': bdm_emails, 'submission_output': submission_output})
...
def send_weekly_bdm_jobs(request, country='India', isProd=False):
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    start_date, end_date = utils.get_weekly_report_start_and_end_datetime(country)
    ...
    queryset = Candidates.objects.raw(
        "SELECT j.id ,j.job_title , j.job_id , cl.company_name as client_name , CONCAT(u.first_name,' ' ,u.last_name)"
        "as bdm_name,u.country as bdm_location, j.job_posted_date, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate, (SELECT GROUP_CONCAT(CONCAT(ca.first_name,' ' ,ca.last_name)) FROM candidates_jobs_stages as "
        "cjs, osms_candidates as ca WHERE ca.id = cjs.candidate_name_id AND j.id = cjs.job_description_id ) as "
        "candidates_name, (SELECT GROUP_CONCAT(cjs.submission_date)  FROM candidates_jobs_stages as cjs, "
        "osms_candidates as ca WHERE ca.id = cjs.candidate_name_id AND j.id = cjs.job_description_id ) as "
...
def send_last_48hours_bdm_jobs(request, country='India', isProd=False):
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    ...
    if queryset is not None:
        ...
        no_jobs_posted_user = User.objects.raw(
            "SELECT u.id, u.first_name FROM users_user as u WHERE u.role = 9 AND u.is_active = 1 AND u.country IN %s "
            "AND u.id NOT IN (SELECT u.id FROM osms_job_description as j, osms_clients as cl,users_user as u WHERE "
            "u.id = j.created_by_id AND j.job_posted_date >= %s AND j.job_posted_date <= %s AND u.country IN %s )",
        ...
        message = render_to_string('bdm__jobs_posted_in_last_48_hours.html', {'data': finalOutput, 'jobs_posted': jobs_posted})
        email = EmailMessage(subject="Jobs Posted by BDMs in Last 48 Hours: ({0} to {1})".format(str(start_date).split(' ')[0],
                                                                                     str(today_date).split(' ')[0]),
                             body=message,
                             from_email='osms.opallios@gmail.com', to=bdm_emails)
        email.content_subtype = 'html'
