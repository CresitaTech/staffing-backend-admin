...
from vendors.models import vendorModel, vendorEmailTemplateModel, emailConfigurationModel, VendorListModel, \
    VendorListDataModel, MailEventsModel, EmailModel
from vendors.serializers import VendorSerializer, EmailTemplateSerializer, EmailConfigurationSerializer, \
    VendorListSerializer, VendorListWriteSerializer, VendorListDataSerializer, VendorListMiniSerializer
            ...
            if len(email_objects) > 0:
                try:
                    ...
                    for email_object in email_objects:
                        ...
                        email = EmailModel()
                        email.subject = getattr(email_object, 'email_subject')
                        ...
                        sender = EmailMessage(subject=getattr(email_object, 'email_subject'),
            ...
            if data_type == "Candidate":
                ...
                for vendor_id in list_data:
                    ...
                    model_instances.append(VendorListDataModel(
                        ...
                        record_id=str(vendor_id).replace("-", ""),
                        email_body=body,
                ...
                logger.info(data_type)
                file = request.FILES['upload_file'].file
                ...
                logger.info("in in rechurn")
                logger.info(request.data['list_id'])
                vendorObj = VendorListModel.objects.get(id=request.data['list_id'])
                logger.info(vendorObj.data_type)
                if vendorObj.data_type == "Custom":
                    model_instances = []
                    file = "/home/admin/projectDir/staffingapp/media/" + str(vendorObj.upload_file)
                    reader = pd.read_csv(file)
                    for _, row in reader.iterrows():
                        logger.info(row)
...
class GetVendorListData(viewsets.ModelViewSet):
    ...
    search_fields = ['template__template_name', 'list__list_name', 'email_to']
    # , 'vendor__contact_person_first_name',
    #                      'vendor__contact_person_last_name', 'vendor__primary_email'
