...
class tataCallsWebhook(viewsets.ModelViewSet):
    ...
    def post(self, request, format=None):
        request.data['caller_id_number'] = request.data[-10:]
        logger.info('caller_id_number: ' + str(request.data['caller_id_number']))

        request.data['call_to_number'] = request.data[-10:]
        logger.info("call_to_number", str(request.data['call_to_number']))
        logger.info("Tata calls Webhook Request Data: " + str(request.data))
        serializeObj = AgentCallsDataModelSerializer(data=request.data)
        if serializeObj.is_valid():
            recruiter = serializeObj.save()
            logger.info(recruiter.id)
            return Response(serializeObj.data, status=status.HTTP_201_CREATED)
        return Response(serializeObj.errors, status=status.HTTP_400_BAD_REQUEST)
