...
from analytics.utils import get_training_data, readPdf, readDocx, readDoc
...
class RecommendedCandidatesViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        # queryset = Candidates.objects.raw("SELECT id, resume from osms_candidates")
        ...
        for i in range(len(serializer.data)):
            rows = {
                'id': str(serializer.data[i]['id']),
                'resume': serializer.data[i]['resume']
            }
            output.append(rows)

        df = pd.DataFrame(output)
        logger.info(df.resume)"""
...
class RecommendedJobsViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        logger.info("Extract content: " + str(content))
