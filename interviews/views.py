...
class CandidateInterviewsList(generics.ListAPIView):
    queryset = interviewsModel.objects.none()

    def get(self, request, format=None):
        if request.query_params.get('candidate_id'):
            candidate_id = str(request.query_params.get('candidate_id')).replace('-', '')
            interviews = interviewsModel.objects.filter(candidate_name_id=candidate_id )
