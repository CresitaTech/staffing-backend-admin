...
from reports.serializers import RecruiterPerformanceSummarySerializer, RecruiterPerformanceSummaryGraphSerializer, \
    ...
    TotalRecordSerializer ,BdmEmailReportSerializer,RecruiterEmailReportSerializer,RecruitersPerformanceSummarySerializer, AssingedDashboardListSerializer ,ClientRevenueSerializer, JobSummaryTableSerializer ,MyCandidateSerializer,UnassignedJobsSerializer, JobsByBDMTableSerializer,JobsByBDMGraphSerializer,ActiveJobsAgingSerializer,AssingedDashboardSerializer
...
class RecruiterPerformanceSummaryTable(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  ...
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date > %s", [today])
            elif date_range == 'yesterday':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date > %s AND cjs.submission_date < %s", [yesterday , today])
            elif date_range == 'week':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [week_start ,week_end])
            elif date_range == 'month':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [month_start ,month_end])
            elif start_date is not None and end_date is not None:
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [start_date ,end_date])
...
class RecruiterPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  ...
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date > %s", [today])
            elif date_range == 'yesterday':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, ca.created_at FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date > %s AND cjs.submission_date < %s", [yesterday , today])
            elif date_range == 'week':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, ca.created_at FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [week_start ,week_end])
            elif date_range == 'month':
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, ca.created_at FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [month_start ,month_end])
            elif start_date is not None and end_date is not None:
                queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                  " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, ca.created_at FROM `osms_job_description` as j, "
                  "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s", [start_date ,end_date])
        ...
        writer = csv.DictWriter(response, fieldnames=["candidate_name", "primary_email", "primary_phone_number",
                  ...
                  "min_rate","max_rate" , "min_salary", "max_salary", "client_name", "location","visa" , "job_type" , "bdm_name",
...
class BdmPerformanceSummaryGraph(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        queryset = None
        print(GLOBAL_ROLE.get('ADMIN'))
        if userGroup is not None and (userGroup == GLOBAL_ROLE.get('ADMIN') or userGroup == GLOBAL_ROLE.get('BDMMANAGER') or userGroup == GLOBAL_ROLE.get('RECRUITERMANAGER')):
            if date_range == 'today':
                queryset = Candidates.objects.raw("SELECT c.id, COUNT(*) as total_count, s.stage_name as stage_name, COUNT(s.stage_name = 'Submission') as submission , u.first_name as first_name, j.created_by_id FROM `osms_candidates` as c, `candidates_stages` as s, `users_user` as u, " 
                "`osms_job_description` as j , `candidates_jobs_stages` as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = j.id AND j.created_by_id = u.id AND cjs.candidate_name_id = c.id AND cjs.submission_date > %s GROUP BY u.first_name , cjs.stage_id", [today])
            elif date_range == 'yesterday':
                queryset = Candidates.objects.raw("SELECT c.id, COUNT(*) as total_count, s.stage_name as stage_name, COUNT(s.stage_name = 'Submission') as submission , u.first_name as first_name, j.created_by_id FROM `osms_candidates` as c, `candidates_stages` as s, `users_user` as u, " 
                "`osms_job_description` as j , `candidates_jobs_stages` as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = j.id AND j.created_by_id = u.id AND cjs.candidate_name_id = c.id AND cjs.submission_date > %s AND cjs.submission_date < %s GROUP BY u.first_name , cjs.stage_id", [yesterday , today])
            elif date_range == 'week':
                queryset = Candidates.objects.raw("SELECT c.id, COUNT(*) as total_count, s.stage_name as stage_name, COUNT(s.stage_name = 'Submission') as submission , u.first_name as first_name, j.created_by_id FROM `osms_candidates` as c, `candidates_stages` as s, `users_user` as u, " 
                "`osms_job_description` as j , `candidates_jobs_stages` as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = j.id AND j.created_by_id = u.id AND cjs.candidate_name_id = c.id AND cjs.submission_date >= %s AND cjs.submission_date <= %s  GROUP BY u.first_name , cjs.stage_id", [week_start , week_end])
            elif date_range == 'month':
                queryset = Candidates.objects.raw("SELECT c.id, COUNT(*) as total_count, s.stage_name as stage_name, COUNT(s.stage_name = 'Submission') as submission , u.first_name as first_name, j.created_by_id FROM `osms_candidates` as c, `candidates_stages` as s, `users_user` as u, " 
                "`osms_job_description` as j , `candidates_jobs_stages` as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = j.id AND j.created_by_id = u.id AND cjs.candidate_name_id = c.id AND cjs.submission_date >= %s AND cjs.submission_date <= %s  GROUP BY u.first_name , cjs.stage_id", [month_start , month_end])
            elif start_date is not None and end_date is not None:
                queryset = Candidates.objects.raw("SELECT c.id, COUNT(*) as total_count, s.stage_name as stage_name, COUNT(s.stage_name = 'Submission') as submission , u.first_name as first_name, j.created_by_id FROM `osms_candidates` as c, `candidates_stages` as s, `users_user` as u, " 
                "`osms_job_description` as j , `candidates_jobs_stages` as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = j.id AND j.created_by_id = u.id AND cjs.candidate_name_id = c.id AND cjs.submission_date >= %s AND cjs.submission_date <= %s GROUP BY u.first_name , cjs.stage_id", [start_date , end_date])
...
class JobSummaryTable(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = Candidates.objects.raw("SELECT * FROM (SELECT j.id ,j.job_id as Job_ID, j.job_title as job_title, cl.company_name AS client_name,j.employment_type,j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate, "
...
class AggregateData(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            ...
            placed = candidatesJobDescription.objects.filter(stage_id__stage_name="Placed").annotate(Count('stage')).count()
            ...
            queryset = jobModel.objects.raw("SELECT c.id, COUNT(*) as total_records FROM `osms_candidates` as c, `osms_job_description` as j, `candidates_stages` as s , `candidates_jobs_stages` as cjs WHERE cjs.job_description_id = j.id AND cjs.stage_id = s.id AND cjs.candidate_name_id = c.id AND j.created_by_id = %s AND s.stage_name != 'Candidate Added' ",[uid])
            ...
            queryset = jobModel.objects.raw("SELECT c.id, COUNT(*) as total_records FROM `osms_candidates` as c, `osms_job_description` as j, `candidates_stages` as s , `candidates_jobs_stages` as cjs WHERE cjs.job_description_id = j.id AND cjs.stage_id = s.id AND cjs.candidate_name_id = c.id AND j.created_by_id = %s AND s.stage_name = 'Placed'", [uid])
            serializer = TotalRecordSerializer(queryset, many=True)
            if serializer.data[0] is not None and len(serializer.data[0]) > 0:
            ...
            submissions = candidatesJobDescription.objects.filter((Q(candidate_name__created_by_id=request.user.id))).exclude(stage__stage_name = "Candidate Added").annotate(Count('stage')).count()
            placed = candidatesJobDescription.objects.filter((Q(created_by_id__created_by_id=request.user.id) | Q(created_by_id=request.user.id)), stage_id__stage_name="Placed").annotate(Count('stage')).count()
            ...
            sendOut = candidatesJobDescription.objects.filter(candidate_name__created_by_id=request.user.id, stage_id__stage_name="SendOut").annotate(
                Count('stage')).count()
            submissions = candidatesJobDescription.objects.filter(candidate_name__created_by_id=request.user.id).exclude(stage__stage_name = "Candidate Added").annotate(Count('stage')).count()
            placed = candidatesJobDescription.objects.filter(candidate_name__created_by_id=request.user.id, stage_id__stage_name="Placed").annotate(
                Count('stage')).count()
...
class TopClients(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            queryset = clientModel.objects.raw("SELECT cl.id, cl.company_name as client_name, (Select COUNT(s.stage_name) from osms_candidates as c,"
            ...
            queryset = clientModel.objects.raw("SELECT cl.id, cl.company_name as client_name, (Select COUNT(s.stage_name) from osms_candidates as c, candidates_stages as s, osms_job_description as j , candidates_jobs_stages as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND s.stage_name != 'Placed' AND cjs.job_description_id = j.id and cjs.candidate_name_id = c.id and j.created_by_id = %s AND j.client_name_id = cl.id ) as submissions, (Select COUNT(s.stage_name) from osms_candidates as c, candidates_stages as s, osms_job_description as j , candidates_jobs_stages as cjs WHERE cjs.stage_id = s.id AND s.stage_name = 'Placed' AND cjs.job_description_id = j.id and cjs.candidate_name_id = c.id and j.created_by_id = %s AND j.client_name_id = cl.id ) as placed, (Select COUNT(j.job_title) from osms_job_description as j WHERE j.created_by_id = %s AND j.client_name_id = cl.id ) as job_title FROM `osms_clients` as cl WHERE cl.created_by_id = %s", [uid, uid, uid, uid])

        elif userGroup is not None and userGroup == GLOBAL_ROLE.get('RECRUITERMANAGER'):
            queryset = clientModel.objects.raw("SELECT cl.id, cl.company_name as client_name, (Select COUNT(s.stage_name) from osms_candidates as c, candidates_stages as s, osms_job_description as j , candidates_jobs_stages as cjs WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND s.stage_name != 'Placed' AND cjs.job_description_id = j.id and j.client_name_id = cl.id and cjs.candidate_name_id = c.id ) as submissions, (Select COUNT(s.stage_name) from osms_candidates as c, candidates_stages as s, osms_job_description as j , candidates_jobs_stages as cjs WHERE cjs.stage_id = s.id AND s.stage_name = 'Placed' AND cjs.job_description_id = j.id and j.client_name_id = cl.id and cjs.candidate_name_id = c.id ) as placed, (Select COUNT(j.job_title) from osms_job_description as j WHERE j.client_name_id = cl.id ) as job_title FROM `osms_clients` as cl, `osms_job_description` as ja WHERE ja.client_name_id = cl.id AND ja.default_assignee_id = %s GROUP BY cl.company_name", [uid])
...
class MyCandidatesList(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('RECRUITER'):
            queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name,"
            ...
            " AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND ca.created_by_id = %s ORDER BY ca.created_at DESC" , [uid])
...
class AssingedJobsList(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            ...
            if request.query_params.get('action'):
                queryset = jobModel.objects.raw("SELECT j.* FROM `osms_job_description` as j, `osms_clients` as cl, `users_user` as u, `job_description_assingment` as ja WHERE (j.client_name_id = cl.id AND u.id = j.default_assignee_id AND ja.job_id_id = j.id AND ja.primary_recruiter_name_id = %s) OR"
                "(j.client_name_id = cl.id AND u.id = j.default_assignee_id AND ja.job_id_id = j.id AND ja.secondary_recruiter_name_id = %s) ORDER BY j.created_at DESC" , [uid, uid])
                page = self.paginate_queryset(queryset)
                if page is not None:
                    serializer = AssingedDashboardListSerializer(queryset, many=True)
                    return self.get_paginated_response(serializer.data)
        ...
        if request.query_params.get('bdm_id') and request.query_params.get('stage'):
            ...
            if date_range == 'today':
                candObj = candidatesJobDescription.objects.filter(job_description__created_by_id = bdm_id ,
                                                    stage__stage_name = request.query_params.get('stage') , submission_date__gt = today)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'yesterday':
                candObj = candidatesJobDescription.objects.filter(job_description__created_by_id = bdm_id ,
                                                    stage__stage_name = request.query_params.get('stage') , submission_date__gt = yesterday , submission_date__lt = today )
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'week':
                candObj = candidatesJobDescription.objects.filter(job_description__created_by_id = bdm_id ,
                                                    stage__stage_name = request.query_params.get('stage') , submission_date__gt = week_start , submission_date__lt = week_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'month':
                candObj = candidatesJobDescription.objects.filter(job_description__created_by_id = bdm_id ,
                                                    stage__stage_name = request.query_params.get('stage') , submission_date__gt = month_start , submission_date__lt = month_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif start_date is not None and end_date is not None:
                candObj = candidatesJobDescription.objects.filter(job_description__created_by_id = bdm_id ,
                                                    stage__stage_name = request.query_params.get('stage') , submission_date__gt = start_date , submission_date__lt = end_date)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
            ...
            if date_range == 'today':
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id = recruiter_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , submission_date__gt = today)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'yesterday':
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id = recruiter_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , submission_date__gt = yesterday ,submission_date__lt = today )
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'week':
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id = recruiter_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , submission_date__gt = week_start , submission_date__lt = week_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'month':
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id = recruiter_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , submission_date__gt = month_start , submission_date__lt = month_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif start_date is not None and end_date is not None:
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id = recruiter_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , submission_date__gt = start_date , submission_date__lt = end_date)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
            ...
            if date_range == 'today':
                candObj = candidatesJobDescription.objects.filter(job_description_id = job_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , job_description__created_at__gt = today)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'yesterday':
                candObj = candidatesJobDescription.objects.filter(job_description_id = job_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , job_description__created_at__gt = yesterday ,job_description__created_at__lt = today )
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'week':
                candObj = candidatesJobDescription.objects.filter(job_description_id = job_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , job_description__created_at__gt = week_start , job_description__created_at__lt = week_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'month':
                candObj = candidatesJobDescription.objects.filter(job_description_id = job_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , job_description__created_at__gt = month_start , job_description__created_at__lt = month_end)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif start_date is not None and end_date is not None:
                candObj = candidatesJobDescription.objects.filter(job_description_id = job_id ,
                                                stage__stage_name = str(request.query_params.get('stage')) , job_description__created_at__gt = start_date , job_description__created_at__lt = end_date)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                return Response(serializer.data)
            ...
            if date_range == 'today':
                ...
                candObj = jobModel.objects.filter(created_by_id = job_creater_id , created_at__gt = yesterday , created_at__lt = today )
                serializer = JobDescriptionSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'week':
                candObj = jobModel.objects.filter(created_by_id = job_creater_id , created_at__gt = week_start , created_at__lt = week_end)
                serializer = JobDescriptionSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif date_range == 'month':
                candObj = jobModel.objects.filter(created_by_id = job_creater_id , created_at__gt = month_start ,created_at__lt = month_end)
                serializer = JobDescriptionSerializer(candObj, many=True)
                return Response(serializer.data)
                ...
                candObj = jobModel.objects.filter(created_by_id = job_creater_id , created_at__month = month ,created_at__year = year)
                serializer = JobDescriptionSerializer(candObj, many=True)
                return Response(serializer.data)
                
            elif request.query_params.get('single_date'):
                candObj = jobModel.objects.filter(created_by_id = job_creater_id , created_at__date = request.query_params.get('single_date'))
                serializer = JobDescriptionSerializer(candObj, many=True)
                return Response(serializer.data)
...
class JobsByBDMSummaryGraph(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = jobModel.objects.raw("SELECT a.id, a.created_at, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id , COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at > %s GROUP BY DATE(a.created_at), bdm_name" , [today])
            elif date_range == 'yesterday':
                queryset = jobModel.objects.raw("SELECT a.id, a.created_at, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id, COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at > %s AND a.created_at < %s GROUP BY DATE(a.created_at), bdm_name" , [yesterday , today])
            elif date_range == 'week':
                queryset = jobModel.objects.raw("SELECT a.id, a.created_at, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id, COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at >= %s AND a.created_at <= %s GROUP BY DATE(a.created_at), bdm_name" , [week_start,week_end])
            elif date_range == 'month':
                queryset = jobModel.objects.raw("SELECT a.id, a.created_at, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id, COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at >= %s AND a.created_at <= %s GROUP BY DATE(a.created_at), bdm_name" , [month_start ,month_end])
            elif start_date is not None and end_date is not None:
                if (datetime.datetime.strptime(end_date, '%Y-%m-%d').date() - datetime.datetime.strptime(start_date, '%Y-%m-%d').date()).days > 30:
                    queryset = jobModel.objects.raw("SELECT a.id, MONTHNAME(a.created_at) AS month, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id, COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at >= %s AND a.created_at <= %s GROUP BY MONTHNAME(a.created_at) , bdm_name" , [start_date , end_date])
                else:
                    queryset = jobModel.objects.raw("SELECT a.id, a.created_at, CONCAT(u.first_name, ' ' , u.last_name) AS bdm_name ,a.created_by_id as bdm_id, COUNT(a.id) AS total_count FROM osms_job_description as a , users_user as u WHERE u.id = a.created_by_id AND a.created_at >= %s AND a.created_at <= %s GROUP BY DATE(a.created_at), bdm_name" , [start_date, end_date])
    ...
    def get(self, request, format=None):
        ...
        if request.query_params.get('bdm_id') and request.query_params.get('stage'):
            ...
            for i in range(array_length):  # Use `xrange` for python 2.
                ...
                rows = {
                    'candidate_name': str(serializer.data[i]['candidate_name']['first_name'])+ ' '+ str(serializer.data[i]['candidate_name']['last_name']),
            ...
            if date_range == 'today':
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id=recruiter_id,
                                                                  stage__stage_name=str(request.query_params.get('stage')),
                                                                  submission_date__gt=today)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
                ...
                candObj = candidatesJobDescription.objects.filter(candidate_name__created_by_id=recruiter_id,
                                                                  stage__stage_name=str(request.query_params.get('stage')),
                                                                  submission_date__gt=yesterday, submission_date__lt=today)
                serializer = CandidateJobsStagesSerializer(candObj, many=True)
...
class ClientRevenueReport(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = jobModel.objects.raw("SELECT cl.id , cl.company_name as client_name_value , SUM(j.projected_revenue) as expected_revenue, (SELECT SUM(js.projected_revenue) FROM osms_job_description as js ,candidates_jobs_stages as cjs, candidates_stages as s WHERE js.client_name_id = cl.id ANd cjs.job_description_id = js.id"
...
def send_recruiter_email(request , country='US'):
    today_date_time = utils.get_current_datetime()
    today_date = utils.get_current_date()
    if country == 'US':
        start_date = datetime.datetime.strptime(today_date_time, '%Y-%m-%d %H:%M:%S') - datetime.timedelta(hours=12, minutes=30, seconds=00)
        end_date = datetime.datetime.strptime(today_date, '%Y-%m-%d') + datetime.timedelta(hours=11, minutes=59, seconds=59)
    queryset = Candidates.objects.raw("SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, cl.company_name AS client_name ,"
    ...
    "j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND u2.country = %s ORDER BY recruiter_name" , [start_date,country])
    output = []
    print(queryset.query)
    if queryset is not None:
        print(str(queryset.query))
        serializer = RecruiterEmailReportSerializer(queryset, many=True)
        userqueryset = User.objects.raw("SELECT id, email, country from users_user where role = '3' AND country = %s", [country])
        ...
        bdmqueryset = User.objects.raw(
            "SELECT id, email, country from users_user where role = '2' AND country = %s", [country])
        ...
        email = EmailMessage(subject="Daily Subimissions for {0}".format(today_date), body=message, from_email='osms.opallios@gmail.com', to=test_email)
        email.content_subtype = 'html'
        #email.cc = ['mathurp@opallios.com','girish@opallios.com','paradkaro@opallios.com','minglaniy@opallios.com','kuriwaln@opallios.com' , 'pandeym@cresitatech.com'] + bdm_emails
...
def send_bdm_email(country):
    ...
    print(today)
    if country == 'US':
        today = datetime.datetime.strptime(today, '%Y-%m-%d %H:%M:%S') - datetime.timedelta(hours=12, minutes=30, seconds=00)
    queryset = Candidates.objects.raw("SELECT j.id ,j.job_title , j.job_id , cl.company_name as client_name , CONCAT(u.first_name,' ' ,u.last_name)"
    " as bdm_name,u.country as bdm_location FROM osms_job_description as j, osms_clients as cl,users_user as u WHERE cl.id = j.client_name_id"
    " AND u.id = j.created_by_id AND j.job_posted_date >= %s AND u.country = %s ORDER BY bdm_name", [today , country])
    output = []
    if queryset is not None:
        print(str(queryset.query))
        ...
        email = EmailMessage(subject="Daily Jobs for {0}".format(today), body=message, from_email='osms.opallios@gmail.com', to=bdm_emails)
        email.content_subtype = 'html'
        email.cc = ['mathurp@opallios.com','girish@opallios.com','paradkaro@opallios.com','minglaniy@opallios.com','kuriwaln@opallios.com']
        email.send()
        return str(queryset.query)