...
from reports.serializers import RecruiterPerformanceSummarySerializer, WeekendEmailReportSerializer, \
    ...
    JobSubmissionsByClientTableSerializer, JobSubmissionsByClientGraphSerializer
...
def send_recruiter_email(country='India', isProd=False):
    ...
    queryset = Candidates.objects.raw(
        ...
        "CONCAT(u1.first_name,' ',u1.last_name) as bdm_name , CONCAT(u2.first_name,' ',u2.last_name) as recruiter_name , u2.country as location , j.job_title, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate FROM"
    ...
    if queryset is not None:
        ...
        no_submission_user = User.objects.raw("SELECT u.id, u.first_name FROM users_user as u WHERE u.role = 3 AND u.is_active = 1 AND u.country = %s "
            "AND u.id NOT IN (SELECT cjs.created_by_id FROM `candidates_jobs_stages` as cjs INNER JOIN "
            "users_user as u ON u.id = cjs.created_by_id WHERE u.role = 3 "
            "AND cjs.submission_date >= %s and cjs.submission_date <= %s "
            "GROUP BY cjs.created_by_id)", [country, start_date, end_date])
...
def send_bdm_email(country='India', isProd=False):
    ...
    queryset = Candidates.objects.raw(
        ...
        "as bdm_name,u.country as bdm_location, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate FROM osms_job_description as j, osms_clients as cl,users_user as u "
    ...
    if queryset is not None:
        ...
        # return render(request, "bdm_email_report.html", {'data': output})
...
def send_recruiter_summary_report(country='India', isProd=False):
    ...
    queryset = Candidates.objects.raw(
        ...
        "`osms_job_description` as j,`users_user` as u1, users_user as u2 , `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, "
        "`candidates_jobs_stages` as cjs WHERE j.created_by_id = u1.id AND ca.created_by_id = u2.id AND s.stage_name = 'Candidate Added' AND "
        "j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND ca.created_at >= %s AND ca.created_at <= %s AND u2.country IN %s ORDER BY recruiter_name",
        [start_date, end_date, user_countries])
    ...
    if queryset is not None:
        ...
        message = render_to_string('recruiter_summary_report.html', {'data': finalOutput})
...
def send_weekly_recruiter_submission(country='India', isProd=False):
    ...
    queryset = Candidates.objects.raw(
        ...
        "recruiter_name , u2.country as location , j.job_title, cjs.submission_date, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate  FROM "
...
def send_weekly_bdm_jobs(country='India', isProd=False):
    ...
    queryset = Candidates.objects.raw(
        "SELECT j.id ,j.job_title , j.job_id , cl.company_name as client_name , CONCAT(u.first_name,' ' ,u.last_name)"
        "as bdm_name,u.country as bdm_location, j.job_posted_date, j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate  FROM osms_job_description as j, osms_clients as cl,"
