...
class JobSummaryTable(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if date_range == 'today':
            queryset = get_jobs_summary_csv_by_date_range_and_id(today, today, bdm_id, country, status)
        elif date_range == 'yesterday':
            queryset = get_jobs_summary_csv_by_date_range_and_id(yesterday, yesterday, bdm_id, country, status)
        elif date_range == 'last-24-hours':
            start_date = str(datetime.datetime.today() - datetime.timedelta(hours=24, minutes=0))
            queryset = get_jobs_summary_csv_by_date_range_and_id(start_date, today, bdm_id, country, status)
        elif date_range == 'week':
            queryset = get_jobs_summary_csv_by_date_range_and_id(week_start, week_end, bdm_id, country, status)
        elif date_range == 'month':
            queryset = get_jobs_summary_csv_by_date_range_and_id(month_start, month_end, bdm_id, country, status)
        elif start_date is not None and end_date is not None:
            queryset = get_jobs_summary_csv_by_date_range_and_id(start_date, end_date, bdm_id, country, status)
...
class JobSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if date_range == 'today':
            ...
            queryset = get_jobs_summary_csv_by_date_range_and_id(yesterday, today, bdm_id, country, status)
        elif date_range == 'last-24-hours':
            start_date = str(datetime.datetime.today() - datetime.timedelta(hours=24, minutes=0))
            queryset = get_jobs_summary_csv_by_date_range_and_id(start_date, today, bdm_id, country, status)
        elif date_range == 'week':
