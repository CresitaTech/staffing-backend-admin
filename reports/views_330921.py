...
def send_recruiter_email():
    ...
    "CONCAT(u1.first_name,' ',u1.last_name) as bdm_name , CONCAT(u2.first_name,' ',u2.last_name) as recruiter_name , u2.country as location FROM"
    ...
    "j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s ORDER BY recruiter_name" , [today])
    output = []
    if queryset is not None:
        print(str(queryset.query))
        ...
        email = EmailMessage(subject="Daily Subimissions for {0}".format(today), body=message, from_email='osms.opallios@gmail.com', to=['pandeym@cresitatech.com' ,'kuriwaln@opallios.com'])
...
def send_bdm_email():
    ...
    " AND u.id = j.created_by_id AND j.created_at >= %s ORDER BY bdm_name", [today])
    output = []
    if queryset is not None:
        print(str(queryset.query))
        ...
        email = EmailMessage(subject="Daily Jobs for {0}".format(today), body=message, from_email='osms.opallios@gmail.com', to=['pandeym@cresitatech.com' ,'kuriwaln@opallios.com'])
        email.content_subtype = 'html'
        email.cc = ['mathurp@opallios.com','girish@opallios.com','paradkaro@opallios.com','minglaniy@opallios.com']
        email.send()
        return Response(data={'msg': 'Success'})