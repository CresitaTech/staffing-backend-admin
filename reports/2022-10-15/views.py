...
from reports.utils import sortDic
...
class RecruiterPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
            ...
            if date_range == 'today':
                ...
                if recruiter_id is not None:
                    queryset = Candidates.objects.raw(
                        "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, j.end_client_name as company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary,"
                        " cl.company_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id, cjs.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, ca.created_at, cl.country FROM `osms_job_description` as j, "
                        "`users_user` as u, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND cjs.submission_date >= %s AND cjs.submission_date <= %s AND ca.created_by_id = %s",
        ...
        writer = csv.DictWriter(response, fieldnames=["candidate_name", "primary_email", "primary_phone_number",
                                                      ...
                                                      "status", "recruiter_name", "client_country", "created_by", "submission_date", "job_date",
                                                      "created_at"
                                                      ])
...
class CandidateCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        writer = csv.DictWriter(response, fieldnames=["candidate_name", "primary_email", "primary_phone_number",
                                                      ...
                                                      "min_rate", "max_rate", "min_salary", "max_salary", "client_name",
                                                      "location", "visa", "job_type", "bdm_name",
                                                      "status", "recruiter_name", "client_country", "is_resume_available", "created_by", "submission_date", "job_date",
                                                      "created_at"
                                                      ])
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            ...
            if date_range == 'today':
                ...
                queryset = Candidates.objects.raw(
                    "SELECT u.id, j.created_by_id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate , ca.max_rate, ca.min_salary , ca.max_salary, j.end_client_name AS client_name, j.location, "
                    "CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, (SELECT ja.created_at as assingment_date FROM job_description_assingment ja WHERE ja.job_id_id = j.id ORDER BY ja.created_at ASC LIMIT 1) as first_assingment_date, cl.country  FROM `osms_job_description` as j, `users_user` as u, `osms_clients` as cl, `osms_candidates` as ca,"
                    "`candidates_stages` as s , `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND cjs.candidate_name_id = ca.id AND cjs.submission_date >= %s AND cjs.submission_date <= %s ",
                    [start_date, end_date])
...
class BdmPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
            ...
            if date_range == 'today':
                ...
                if bdm_id is not None:
                    queryset = Candidates.objects.raw(
                        "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, j.end_client_name as company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate , ca.max_rate, ca.min_salary , ca.max_salary, cl.company_name AS client_name, j.location, "
                        "CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cl.country, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type,ca.created_at , (SELECT ja.created_at as assingment_date FROM job_description_assingment ja WHERE ja.job_id_id = j.id ORDER BY ja.created_at ASC LIMIT 1) as first_assingment_date, (SELECT cas.activity_status FROM `candidates_activity_status` as cas WHERE cas.candidate_name_id = ca.id ORDER BY `created_at` DESC LIMIT 1) as activity_status, (SELECT cas.created_at FROM `candidates_activity_status` as cas WHERE cas.candidate_name_id = ca.id ORDER BY `created_at` DESC LIMIT 1) as last_updated_date FROM `osms_job_description` as j, `users_user` as u, `osms_clients` as cl, `osms_candidates` as ca,"
                        "`candidates_stages` as s , `candidates_jobs_stages` as cjs WHERE j.created_by_id = u.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND cjs.candidate_name_id = ca.id AND cjs.submission_date >= %s AND cjs.submission_date <= %s AND j.created_by_id = %s ",
                        [month_start, month_end, bdm_id])
                else:
                    queryset = Candidates.objects.raw(
                        "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, j.end_client_name as company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate , ca.max_rate, ca.min_salary , ca.max_salary, cl.company_name AS client_name, j.location, "
                        "CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id as recruiter_name, cl.country, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type,ca.created_at , (SELECT ja.created_at as assingment_date FROM job_description_assingment ja WHERE ja.job_id_id = j.id ORDER BY ja.created_at ASC LIMIT 1) as first_assingment_date, (SELECT cas.activity_status FROM `candidates_activity_status` as cas WHERE cas.candidate_name_id = ca.id ORDER BY `created_at` DESC LIMIT 1) as activity_status, (SELECT cas.created_at FROM `candidates_activity_status` as cas WHERE cas.candidate_name_id = ca.id ORDER BY `created_at` DESC LIMIT 1) as last_updated_date FROM `osms_job_description` as j, `users_user` as u, `osms_clients` as cl, `osms_candidates` as ca,"
        ...
        writer = csv.DictWriter(response, fieldnames=["candidate_name", "primary_email", "primary_phone_number",
                                                      ...
                                                      "min_rate", "max_rate", "min_salary", "max_salary", "client_name",
                                                      "location", "visa", "job_type", "bdm_name",
                                                      "status", "recruiter_name", "client_country", "submission_date", "job_date",
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = Candidates.objects.raw(
                    ...
                    "CONCAT(u.first_name,' ',u.last_name) as bdm_name, j.created_at AS job_date, cl.country FROM `osms_job_description` as j LEFT JOIN `users_user` as u on j.created_by_id = u.id LEFT JOIN `osms_clients` as cl on j.client_name_id = cl.id WHERE j.created_at > %s )AS"
                ...
                queryset = Candidates.objects.raw(
                    "SELECT * FROM (SELECT j.id ,j.job_id as Job_ID, j.job_title as job_title, cl.company_name AS client_name,j.employment_type,j.min_salary ,j.max_salary ,j.min_rate ,j.max_rate, "
                    "CONCAT(u.first_name,' ',u.last_name) as bdm_name, j.created_at AS job_date, cl.country FROM `osms_job_description` as j LEFT JOIN `users_user` as u on j.created_by_id = u.id LEFT JOIN `osms_clients` as cl on j.client_name_id = cl.id WHERE j.created_at > %s AND j.created_at < %s )AS"
                    " A LEFT JOIN (select j.id as job_id , sum(case when s.stage_name = 'Client Interview' then 1 else 0 end) as Client_Interview, sum(case when s.stage_name = 'Offered' then 1 else 0 end) as Offered , sum(case when s.stage_name != 'Candidate Added' then 1 else 0 end) as Submission ,"
                    " sum(case when s.stage_name = 'Rejected By Team' then 1 else 0 end) as 'Rejected_By_Team' ,sum(case when s.stage_name = 'Sendout Reject' then 1 else 0 end) as 'Sendout_Reject' , sum(case when s.stage_name = 'Offer Rejected' then 1 else 0 end) as 'Offer_Rejected' ,sum(case when s.stage_name = 'Shortlisted'"
                    " then 1 else 0 end) as 'Shortlisted' , sum(case when s.stage_name = 'Internal Interview' then 1 else 0 end) as 'Internal_Interview' ,sum(case when s.stage_name = 'Awaiting Feedback' then 1 else 0 end) as 'Awaiting_Feedback' , sum(case when s.stage_name = 'Placed' then 1 else 0 end) as 'Placed' ,"
                    "sum(case when s.stage_name = 'Rejected By Client' then 1 else 0 end) as 'Rejected_By_Client' ,sum(case when s.stage_name = 'Submission Reject' then 1 else 0 end) as 'Submission_Reject' ,sum(case when s.stage_name = 'SendOut' then 1 else 0 end) as 'SendOut' from `osms_job_description` as j ,"
...
class JobSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        writer = csv.DictWriter(response, fieldnames=['id', 'job_id',
                                                      'job_title', 'client_name', 'bdm_name', 'job_date', 'client_country', 'job_type',
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            ...
            active_jobs = jobModel.objects.filter(
                created_by_id=request.user.id, status="Active").count()  # Q(created_by_id__created_by_id=request.user.id) |
...
class TopClients(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            ...
            queryset = clientModel.objects.raw(
                "SELECT cl.id, cl.company_name as client_name, s.stage_name as stage, (Select COUNT(j.job_title) from osms_job_description as j WHERE j.client_name_id = cl.id ) as job_title, cjs.created_at FROM candidates_stages as s, candidates_jobs_stages as cjs, `osms_clients` as cl, `osms_job_description` as ja WHERE cjs.stage_id = s.id AND s.stage_name != 'Candidate Added' AND cjs.job_description_id = ja.id AND ja.client_name_id = cl.id AND ja.default_assignee_id = %s ORDER BY cjs.created_at DESC",
                [uid])
...
def send_weekly_bdm_jobs_summary(request, country='India', isProd=False):
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    ...
    queryset = Candidates.objects.raw("SELECT * FROM (SELECT j.id ,j.job_title , j.job_id as custom_job_id , cl.company_name "
                                      ...
                                      [start_date, end_date, start_date, end_date, start_date, end_date, start_date, end_date, user_countries, country])
    ...
    if queryset is not None:
        ...
        if isProd:
            ...
            email.cc = ['mathurp@opallios.com', 'kuriwaln@opallios.com'] # 'mathurp@opallios.com', 
...
def send_weeklly_recuiter_performance_report(request, country='India', isProd=False):
    week_start, week_end = utils.week_date_range()
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    start_date, end_date = utils.get_weekly_report_start_and_end_datetime(country)
...
def send_weekly_recruiter_submission(country='US', isProd=False):
    today_date = utils.get_current_datetime()
    # start_date = str(datetime.date.today() - datetime.timedelta(days=7))
    start_date, end_date = utils.get_weekly_report_start_and_end_datetime(country)
    ...
    if queryset is not None:
        ...
        if isProd:
            ...
            email.cc = ['kuriwaln@opallios.com']
            ...
            for bdmName, groupDf in grouped:
                ...
                for item in range(len(statusDf1)):
                    ...
                    if statusDf1.index[item] == 'Submission' or statusDf1.index[item] == 'Submission Reject' or \
                            ...
                            statusDf1.index[item] == 'Internal Interview' or statusDf1.index[item] == 'Internal Interview Reject' or statusDf1.index[item] == 'Candidate Review' or \
                            statusDf1.index[item] == 'Client Interview - First' or \
                            statusDf1.index[item] == 'Interview Backout' or statusDf1.index[item] == 'Interview Reject' or statusDf1.index[item] == 'Interview Select' or \
                            statusDf1.index[item] == 'Client Interview - Second' or statusDf1.index[item] == 'Second Interview Reject' or statusDf1.index[item] == 'Offer Rejected' or \
                            statusDf1.index[item] == 'Offer Backout' or statusDf1.index[item] == 'Feedback Awaited' or statusDf1.index[item] == 'Hold by BDM' or \
                    ...
                    if statusDf1.index[item] == 'SendOut' or statusDf1.index[item] == 'Sendout Reject' or \
                            ...
                            statusDf1.index[item] == 'Interview Reject' or statusDf1.index[item] == 'Interview Select' or statusDf1.index[item] == 'Client Interview - Second' or \
                            statusDf1.index[item] == 'Second Interview Reject' or statusDf1.index[item] == 'Offered' or statusDf1.index[item] == 'Offer Backout' or \
                    ...
                    if statusDf1.index[item] == 'Client Interview - First' or statusDf1.index[item] == 'Interview Backout' or \
                            statusDf1.index[item] == 'Interview Reject' or statusDf1.index[item] == 'Interview Select' or statusDf1.index[item] == 'Client Interview - Second' or \
                            statusDf1.index[item] == 'Second Interview Reject' or statusDf1.index[item] == 'Offer Rejected' or statusDf1.index[item] == 'Offer Backout' or \
                            statusDf1.index[item] == 'Feedback Awaited' or statusDf1.index[item] == 'Hold by Client' or \
                            statusDf1.index[item] == 'Shortlisted' or statusDf1.index[item] == 'Offered' or \
                            statusDf1.index[item] == 'Placed':
                    ...
                    if statusDf1.index[item] == 'Offered' or statusDf1.index[item] == 'Offer Rejected'  or statusDf1.index[item] == 'Offer Backout':
        ...
        if not df.empty:
            ...
            for name, group in grouped:
                ...
                status_tuple = ('Submission', 'Submission Reject', 'SendOut', 'Sendout Reject', 'Internal Interview',
                                'Internal Interview Reject', 'Candidate Review', 'Client Interview - First', 'Interview Backout',
                                'Interview Reject', 'Interview Select', 'Client Interview - Second', 'Second Interview Reject',
