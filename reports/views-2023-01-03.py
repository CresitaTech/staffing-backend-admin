...
from reports.store_procedures_call import get_jobs_summary_csv_by_date_range_and_id, get_candidate_queryset, \
    ...
    get_candidate_details_by_job_id_csv
...
from reports.utils import sortDic, calculateSubmission, calculateSendOut, download_csv
        ...
        """        if userGroup is not None and userGroup == GLOBAL_ROLE.get('ADMIN'):
            if date_range == 'today':
                queryset = Candidates.objects.raw(
                    "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary, j.end_client_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id, CONCAT(u1.first_name,' ',u1.last_name) as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, cl.country, rc.attempted_calls, rc.connected_calls, rc.missed_calls FROM `osms_job_description` as j, `users_user` as u, `users_user` as u1, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs, recruiter_calls as rc WHERE j.created_by_id = u.id AND ca.created_by_id = u1.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND u1.external_user = rc.recruiter_name AND cjs.submission_date >= %s",
                    [today])
            elif date_range == 'yesterday':
                queryset = Candidates.objects.raw(
                    "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary, j.end_client_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id, CONCAT(u1.first_name,' ',u1.last_name) as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, cl.country, rc.attempted_calls, rc.connected_calls, rc.missed_calls FROM `osms_job_description` as j, `users_user` as u, `users_user` as u1, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs, recruiter_calls as rc WHERE j.created_by_id = u.id AND ca.created_by_id = u1.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND u1.external_user = rc.recruiter_name AND cjs.submission_date >= %s AND cjs.submission_date <= %s",
                    [yesterday, today])
            elif date_range == 'week':
                queryset = Candidates.objects.raw(
                    "SELECT ca.id, CONCAT(ca.first_name, ' ' , ca.last_name) as candidate_name, ca.primary_email, ca.primary_phone_number, ca.company_name, ca.total_experience, ca.rank, j.job_title as job_title, ca.skills_1, ca.min_rate,ca.max_rate, ca.min_salary , ca.max_salary, j.end_client_name AS client_name, j.location, CONCAT(u.first_name,' ',u.last_name) as bdm_name, s.stage_name as status, ca.created_by_id, CONCAT(u1.first_name,' ',u1.last_name) as recruiter_name, cjs.submission_date AS submission_date, j.created_at AS job_date ,ca.visa as visa ,j.employment_type as job_type, cl.country, rc.attempted_calls, rc.connected_calls, rc.missed_calls FROM `osms_job_description` as j, `users_user` as u, `users_user` as u1, `osms_clients` as cl, `osms_candidates` as ca, `candidates_stages` as s, `candidates_jobs_stages` as cjs, recruiter_calls as rc WHERE j.created_by_id = u.id AND ca.created_by_id = u1.id AND s.stage_name != 'Candidate Added' AND j.client_name_id = cl.id and j.id = cjs.job_description_id AND cjs.stage_id = s.id AND ca.id = cjs.candidate_name_id AND u1.external_user = rc.recruiter_name AND cjs.submission_date >= %s AND cjs.submission_date <= %s",
...
class RecruiterCallsPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        writer = csv.DictWriter(response, fieldnames=['recruiter_name', 'jobs',
                                                      'country', 'attempted_calls', 'connected_calls', 'missed_calls',
...
class BdmPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if country is not None and country != 'ALL':
            filterCountry = " j.country = '" + country + "' AND "
        filterQuery = filterRecruiter + filterCountry
        print(uid)
...
class JobSummaryTable(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if date_range == 'today':
            queryset = get_jobs_summary_csv_by_date_range_and_id(today, today, bdm_id, country)
        elif date_range == 'yesterday':
            queryset = get_jobs_summary_csv_by_date_range_and_id(yesterday, today, bdm_id, country)
        elif date_range == 'last-24-hours':
            start_date = str(datetime.datetime.today() - datetime.timedelta(hours=24, minutes=0))
            queryset = get_jobs_summary_csv_by_date_range_and_id(start_date, today, bdm_id, country)
        elif date_range == 'week':
            queryset = get_jobs_summary_csv_by_date_range_and_id(week_start, week_end, bdm_id, country)
        elif date_range == 'month':
            queryset = get_jobs_summary_csv_by_date_range_and_id(month_start, month_end, bdm_id, country)
        elif start_date is not None and end_date is not None:
            queryset = get_jobs_summary_csv_by_date_range_and_id(start_date, end_date, bdm_id, country)
        ...
        for i in range(array_length):
            ...
            """            if recruiter_name is not None:
                if assigned_date is not None:
                    candidates_list = recruiter_name.split(",")
                    dates_list = assigned_date.split(",")
                    if len(dates_list) == len(candidates_list):
                        for i in range(len(candidates_list)):
                            assigned_recruiters = str(assigned_recruiters) + str(candidates_list[i]) + ' - ' + str(
...
class JobSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        writer = csv.DictWriter(response, fieldnames=['id', 'job_id',
                                                      'job_title', 'client_name', 'bdm_name', 'job_date',
                                                      'client_country', 'number_of_opening', 'job_type',
...
class JobSummaryTableByDatewiseBreakdownCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if date_range == 'today':
            queryset = get_jobs_summary_by_datewise_breakdown_csv(today, today, job_id)
        elif date_range == 'yesterday':
            queryset = get_jobs_summary_by_datewise_breakdown_csv(yesterday, today, job_id)
        elif date_range == 'last-24-hours':
            start_date = str(datetime.datetime.today() - datetime.timedelta(hours=24, minutes=0))
            queryset = get_jobs_summary_by_datewise_breakdown_csv(start_date, today, job_id)
        elif date_range == 'week':
            queryset = get_jobs_summary_by_datewise_breakdown_csv(week_start, week_end, job_id)
        elif date_range == 'month':
            queryset = get_jobs_summary_by_datewise_breakdown_csv(month_start, month_end, job_id)
        elif start_date is not None and end_date is not None:
            queryset = get_jobs_summary_by_datewise_breakdown_csv(start_date, end_date, job_id)
...
def send_recruiter_email(request, country='India', isProd=False):
