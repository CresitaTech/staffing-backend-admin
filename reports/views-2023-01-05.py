...
class RecruiterCallsPerformanceSummaryCSV(generics.ListAPIView):
    ...
    def get(self, request, format=None):
        ...
        if data_type == 'all_data':
            ...
            if date_range == 'today':
                queryset = get_recruiter_calls_performance_table_alldata_csv(today, today, recruiter_id, country, data_type)
            elif date_range == 'yesterday':
                queryset = get_recruiter_calls_performance_table_alldata_csv(yesterday, today, recruiter_id, country, data_type)
            elif date_range == 'last-24-hours':
                start_date = str(datetime.datetime.today() - datetime.timedelta(hours=24, minutes=0))
                queryset = get_recruiter_calls_performance_table_alldata_csv(start_date, today, recruiter_id, country, data_type)
            elif date_range == 'week':
                queryset = get_recruiter_calls_performance_table_alldata_csv(week_start, week_end, recruiter_id, country, data_type)
            elif date_range == 'month':
                queryset = get_recruiter_calls_performance_table_alldata_csv(month_start, month_end, recruiter_id, country, data_type)
            elif start_date is not None and end_date is not None:
                queryset = get_recruiter_calls_performance_table_alldata_csv(start_date, end_date, recruiter_id, country, data_type)
            ...
            for i in range(array_length):
                rows = {
                    ...
                    'notice_period': serializer.data[i]['notice_period'],
                    'mode_of_interview': serializer.data[i]['mode_of_interview'],
                    'number_of_opening': serializer.data[i]['number_of_opening'],
                    'mode_of_work': serializer.data[i]['mode_of_work'],
                    'projected_revenue': serializer.data[i]['projected_revenue'],
            ...
            writer = csv.DictWriter(response, fieldnames=['recruiter_name', 'jobs', 'country', 'min_salary', 'max_salary', 'max_rate', 'min_rate', 'notice_period', 'mode_of_interview',
                                                          'number_of_opening', 'mode_of_work', 'projected_revenue',
                                                          'job_posted_date', 'updated_at',
            ...
            writer = csv.DictWriter(response, fieldnames=['recruiter_name', 'jobs',
                                                          'country', 'attempted_calls', 'connected_calls',
                                                          'missed_calls',
    ...
    def get(self, request, format=None):
        ...
        for i in range(array_length):
            ...
            rows = {
                ...
                'SubmissionByRecruiter': serializer.data[i]['Submission'],
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="job_summary.csv"'
        writer = csv.DictWriter(response, fieldnames=['job_id', 'client_name', 
                                                      'job_title', 'bdm_name', 'job_date',
                                                      'client_country', 'number_of_opening', 'job_type',
                                                      "SubmissionByRecruiter", "StatusStillSubmission", "SubmissionReject", "SendOut",
                                                      "SendoutReject",
                                                      "ClientInterview", "RejectedByClient", "Shortlisted", "Offered",
                                                      "Placed", "CandidateAdded", "CandidateReview", "RejectedByTeam",
                                                      "ClientInterviewSecond",
