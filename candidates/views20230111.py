...
class CandidateStageViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('BDMMANAGER'):
            queryset = candidateStageModel.objects.filter(type='External')
        else:
            roles_list = [3, 2]
            queryset = candidateStageModel.objects.filter(role__in=roles_list, type='External')
