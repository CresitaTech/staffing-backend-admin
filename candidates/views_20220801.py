...
from candidates.serializers import CandidateSerializer, ActivityStatusSerializer, SubmissionSerializer, \
    ...
    CandidateJobsStagesSerializer, CandidateJobsStagesWriteSerializer
...
class CandidateViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        if userGroup is not None and userGroup == 'ADMIN':
            ...
            if request.query_params.get('action'):
                querysetJS = candidatesJobDescription.objects.values('candidate_name_id').exclude(stage_id__stage_name='Candidate Added').filter(job_description_id__in=jobModel.objects.filter(created_by_id=request.user.id) )
            ...
            if request.query_params.get('action'):
                queryset = Candidates.objects.filter(created_by_id=request.user.id) # exclude(stage__stage_name='Candidate Added').
            else:
                queryset = Candidates.objects.all()
        else:
            queryset = Candidates.objects.none()
...
class CandidateStageViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        if userGroup is not None and userGroup == GLOBAL_ROLE.get('BDMMANAGER'):
            ...
            queryset = candidateStageModel.objects.filter(role=3)
...
class CandidateStageReviewViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        logger.info('candidate_id: ' + request.query_params.get('candidate_id'))
        logger.info('jobdescription_id: ' + request.query_params.get('jobdescription_id'))
        if request.query_params.get('candidate_id') and request.query_params.get('jobdescription_id'):
            queryset = activityStatusModel.objects.filter(activity_status="Candidate Review", candidate_name=request.query_params.get('candidate_id'), job_id=request.query_params.get('jobdescription_id'))
    ...
    def get(self, request, format=None):
        if request.query_params.get('job_id'):
            job_id = str(request.query_params.get('job_id')).replace('UUID', ''). \
                replace('(\'', '').replace('\')', '').replace('-', '')
            queryset = Candidates.objects.raw(
                "SELECT ca.id, ca.first_name , ca.last_name ,CONCAT(u.first_name , ' ' , u.last_name) as updated_by_name , s.stage_name as status , cjs.submission_date ,cjs.updated_at ,ca.max_rate , ca.min_rate ,ca.min_salary , ca.max_salary , ca.visa FROM candidates_jobs_stages as cjs , users_user as u, osms_candidates as ca , candidates_stages as s"
                " WHERE u.id = cjs.updated_by_id AND cjs.stage_id = s.id AND cjs.candidate_name_id = ca.id AND cjs.job_description_id = %s ORDER BY cjs.submission_date DESC",
                [job_id])
        else:
